...
const InputAsyncTagging = React.forwardRef((
  { value, invalid, placeholder, onChange, onBlur, loadOptionApi, isDisabled = false },
  ...
  const loadOptions = (inputValue) =>
    loadOptionApi({ filter: { search: inputValue }, page: 1, size: 10 }).then((t) =>
      t.rows.map(mappingServerTagging),
    )

  const handleChange = (newValue) => {
    console.log("handleChange", newValue)
    onChange(newValue ? newValue.map((t) => ({ id: t.id, label: t.label, color: t.color })) : null)
  }
