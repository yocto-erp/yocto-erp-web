  ...
  const onSelectItem = useCallback(
    (row) => {
      setSelectedList((prevState) => {
        if (prevState[`item${String(row.privateId)}`]) {
          // eslint-disable-next-line no-param-reassign
          delete prevState[`item${String(row.privateId)}`]
          return {
            ...prevState,
          }
        }
        return {
          ...prevState,
          [`item${String(row.privateId)}`]: row.privateId,
        }
      })
    },
    [setSelectedList],
    ...
    (isSelectAll) => {
      if (isSelectAll) {
        ...
        for (let i = 0; i < rows.length; i += 1) {
          newState[`item${String(rows[i].privateId)}`] = rows[i].privateId
        }
