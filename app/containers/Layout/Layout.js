...
import { COMPANY_SCHOOL_UPDATE, COMPANY_SCHOOL_UPDATE_LIST } from "../CompanySchool/constants"
import CompanySchoolUpdate from "../CompanySchool/CompanySchoolUpdate"
import CompanySchoolPage from "../CompanySchool"
        ...
        <main>
          <OverlayScrollbarsComponent
            ...
            options={{
              ...
              callbacks: {
                onScroll: () => {
                  const scrollHeight = scrollRef.current?.osInstance().scroll().position.y || 0
          ...
          >
            <div className="content">
              <Switch>
                ...
                <Route path={`${ADMIN_PATH}/dashboard`} exact component={Dashboard} />
                <Route path={SALE_ORDER_ROOT_PATH} component={SaleOrderPage} />
                <Route path={PURCHASE_ORDER_ROOT_PATH} component={PurchasePage} />
                <Route path={`${WAREHOUSE_ROOT_PATH}`} component={WarehousePage} />
                <Route path={`${PRODUCT_ROOT_PATH}`} component={ProductPage} />
                <Route path={`${INVENTORY_ROOT_PATH}`} component={InventoryPage} />
                ...
                <Route path={FORM_REGISTER_ROOT_PATH} component={FormRegisterSignupPage} />
                <Route path={STUDENT_ROOT_PATH} component={StudentRoutePage} />
                <Route path={`${CONFIGURATION_ROOT_PATH}`} component={ConfigurationPage} />
                <Route path={`${COMPANY_SCHOOL_UPDATE}`} component={CompanySchoolUpdate} />
                <Route path={`${COMPANY_SCHOOL_UPDATE_LIST}`} component={CompanySchoolPage} />
                <Route path={`${TEMPLATE_PRINT_ROOT_PATH}`} component={TemplatePrintPage} />
                <Route path={`${TEMPLATE_EMAIL_ROOT_PATH}`} component={TemplateEmailPage} />
                <Route path={`${SURVEY_MANAGEMENT_ROOT_PATH}`} component={SurveyAdminPage} />
