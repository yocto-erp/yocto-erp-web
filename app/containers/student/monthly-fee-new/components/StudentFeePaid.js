...
const StudentFeePaid = ({ isOpen, onClose, student }) => {
  const {
    ...
    api: (formData) => {
      ...
      return studentMonthlyFeeNewApi.pay(student.id, formData)
    },
    onConfirm: (formData) => ({
  ...
  const {
    ...
    exec,
  } = useApi((std) => studentMonthlyFeeNewApi.printData(std.id))
  ...
  useEffect(() => {
    if (student) {
      reset({
        ...
        remark: "",
        name: `Month ${student.monthFee + 1}/${student.yearFee} tuition fee of ${
  ...
  const { sendEmailConfirm, storeCashIn } = watch(["sendEmailConfirm", "storeCashIn"])

  return (
    <Modal className="primary xx-large" isOpen={isOpen} fade={false} scrollable>
      <ModalHeader toggle={() => onClose(false)}>
        Set Payment for student {student?.student.child.firstName} {student?.student.child.lastName}{" "}
        on {student?.monthFee}-{student?.yearFee}
      </ModalHeader>
