...
const CloneNextMonth = ({ isOpen = false, ids = [], onClose, isUpdated = false }) => {
  ...
  const {
    ...
    api: (formData) =>
      new Promise((resolve) => {
        ...
        history.push(newPage(STUDENT_MONTHLY_NEW_ROOT_PATH), { details })
        resolve(true)
      }),
  })
  ...
  useEffect(() => {
    if (ids && ids.length) {
      studentMonthlyFeeNewApi.read(ids.toString()).then((data) => {
        setResult(data)
      })
    }
  }, [ids])
  ...
  return (
    <Modal isOpen={isOpen} fade={false}>
      <Form onSubmit={onSubmit} noValidate formNoValidate>
        <ModalHeader toggle={() => onClose(false)}>Clone Student Next Month</ModalHeader>
        <ModalBody>
