...
const PaidPage = () => {
  ...
  const {
    ...
    api: (formData) => {
      ...
      return studentMonthlyFeeNewApi.pay(id, formData)
    },
    onConfirm: (formData) => ({
  ...
  useEffect(() => {
    if (state.status === API_STATE.SUCCESS && configure) {
      ...
      const name = `Học phí ${monthFee.student.child.name} (${
        ...
        monthFee.numberOfMonths > 0 ? ` - ${formatMonth(monthFee.toMonth, monthFee.toYear)}` : ""
      }`
      const tuitionFee = monthFee.feePerMonth * (monthFee.numberOfMonths || 1) - monthFee.scholarFee
  ...
  return (
    <>
      <PageTitle
        ...
        }) - Học phí tháng ${formatMonth(monthlyFee?.monthFee, monthlyFee?.yearFee)} ${
